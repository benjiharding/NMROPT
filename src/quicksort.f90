...
contains
   ...
   !Sort a two dimensional array of reals
   integer function double_sort_2d(a, left, right, col, b) result(err)
      ...
      integer, allocatable, target :: ids(:)
      integer, allocatable, target :: tmpids(:)
      integer :: i
      save :: temp, ids, tmpids
      ...
      if (present(b)) then
         if (allocated(tmpids)) then
            if (lbound(tmpids, 1) > left .or. ubound(tmpids, 1) < right) then
               deallocate (tmpids)
               allocate (tmpids(left:right), stat=err)
               if (err /= 0) then
                  return
               end if
            end if
      ...
      call double_sort_2d_(left, right, col, 1, b)

      deallocate (ids, temp)
      if (present(b)) deallocate (tmpids)
   end function double_sort_2d

   !Recursive sort for integer_sort_2d
   recursive subroutine double_sort_2d_(left, right, cols, icol, b)
      integer, intent(in) :: left, right !left and right bounds
      integer, intent(in) :: cols(:) !columns being sorted
      integer, intent(in) :: icol !current column index
      integer, intent(inout), optional :: b(:)
      ...
      do while (i0 < right)
         ...
         !Determine sorted order of elements
         if (icol < ubound(cols, 1)) then
            call double_sort_2d_(i0, i1, cols, icol + 1, b)
         end if
